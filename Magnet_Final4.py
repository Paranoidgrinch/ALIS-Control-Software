...
class DeltaMagnetController(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setFixedSize(900, 800)  # Increased size for new controls
    ...
    def create_ui(self):
        ...
        manual_layout.addWidget(QtWidgets.QLabel("Voltage Limit:"), 3, 0)
        manual_layout.addWidget(self.volt_spin, 3, 1)
    ...
    def send_current_value(self):
        ...
        if self.current_mode == "Manual" and self.sock:
    ...
    def update_measurements(self):
        ...
        if current:
            ...
            # Log the measurement if logging is active
            if self.logging_active and self.log_file:
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                self.log_file.write(f"{timestamp},{current_val:.6f}\n")
                self.log_file.flush()  # Ensure data is written immediately
        
        if voltage:
            self.meas_voltage.setText(f"{float(voltage):.3f} V")
            ...
            if file_path:
                try:
                    ...
                    if os.stat(file_path).st_size == 0:
                        self.log_file.write("Timestamp,Current(A)\n")
