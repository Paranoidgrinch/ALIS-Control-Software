...
class ScrollableSlider(QSlider):
    ...
    def wheelEvent(self, event):
        ...
        step = int(float(self.control['step_selector'].currentText()) * self.control['multiplier'])
...
class OPCControlPanel(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Current Control
        self.current_control = self.create_slider_control(0, 2, 10, "A", default_step=0.1)
    ...
    def create_slider_control(self, min_val, max_val, multiplier, unit, default_step=1.0):
        ...
        slider.setMinimum(min_val * multiplier)
        slider.setMaximum(max_val * multiplier)
        ...
        step_selector.setFixedWidth(50)  # Narrower combo box
        for step in self.allowed_steps:
            step_selector.addItem(f"{step:.1f}")
        ...
        # Value display
        value_label = QLabel(f"{min_val:.1f} {unit}")
        value_label.setStyleSheet("font-weight: bold;")
        value_label.setFixedWidth(80)  # Narrower value display
        ...
        def decrease_value():
            step = int(float(step_selector.currentText()) * multiplier)
    ...
    def update_einzellinse_voltage(self):
        ...
        try:
            ...
            slider_value = int(new_einzellinse_voltage * self.einzellinse_voltage_control['multiplier'])
    ...
    def refresh_all(self):
        ...
        try:
            ...
            self.current_control['slider'].blockSignals(True)
            self.current_control['slider'].setValue(int(current_value * self.current_control['multiplier']))
            self.current_control['slider'].blockSignals(False)
    ...
    def refresh_voltage(self, control):
        ...
        control['slider'].setValue(int(value * control['multiplier']))
